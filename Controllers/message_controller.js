...
const handleIncomingMessage = async (req, res) => {
  try {
    ...
    if (recipient.aiBotEnabled) {
      // Fetch voice recording
      const voiceRecordingUrl = recipient.voiceRecording;

      // Pass message and voiceRecordingUrl to AI module
      const aiResponse = await aiModule.generateResponse({
        senderId,
        receiverId,
        message,
        voiceRecordingUrl
      ...
      res.status(200).json({ message: "Message processed by AI" });
    } else {
      // Handle message if AI bot is not enabled
      // Implement sending message directly here...

      res.status(200).json({ message: "Message sent directly" });
    }
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Internal Server Error");
